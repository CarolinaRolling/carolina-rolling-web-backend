...
// ─── Helper: generate pickup receipt PDF buffer ───────────────────────────────
async function generatePickupReceiptBuffer(workOrder, entry, idx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const pickupDisp = computeDisplayNumbers(workOrder.parts || []).display;
    ...
    regularItems.forEach((item, i) => {
      ...
      if (item.rollingDescription) {
        ...
        doc.text(cleanRoll1, 200, ry, { width: 360 });
        ry += doc.heightOfString(cleanRoll1, { width: 360 }) + 1;
      }
    ...
    ry += 60;
    // Captured digital signature (client signed on-screen) drawn just above the signature line
    if (typeof entry.signature === 'string' && entry.signature.startsWith('data:image')) {
      try {
        const sigBuf = Buffer.from(entry.signature.split(',')[1], 'base64');
        doc.image(sigBuf, 55, ry - 42, { fit: [185, 38] });
      } catch (e) {}
    }
    ...
    if (entry.signature) {
      const sigDate = entry.signedAt ? new Date(entry.signedAt) : (entry.date ? new Date(entry.date) : new Date());
      doc.font('Helvetica').fontSize(9).fillColor('#333').text(fmtDatePT(sigDate) + ' ' + fmtTimePT(sigDate) + ' PT', 300, ry - 12);
    }
...
async function saveShipmentDocument(workOrder, entry, idx) {
  try {
    const fileStorage = require('../utils/storage');
    const drLabel = workOrder.drNumber ? 'DR-' + workOrder.drNumber : (workOrder.orderNumber || workOrder.id);
    const filename = 'Shipment-' + (idx + 1) + '-' + drLabel + '.pdf';
    const existing = await WorkOrderDocument.findOne({ where: { workOrderId: workOrder.id, originalName: filename } });
    if (existing) {
      if (existing.cloudinaryId) try { await fileStorage.deleteFile(existing.cloudinaryId); } catch {}
      await existing.destroy();
    }
    const pdfBuffer = await generatePickupReceiptBuffer(workOrder, entry, idx);
    const uploadResult = await fileStorage.uploadBuffer(pdfBuffer, { folder: 'work-orders/' + workOrder.id + '/documents', filename, mimeType: 'application/pdf' });
    await WorkOrderDocument.create({ workOrderId: workOrder.id, originalName: filename, mimeType: 'application/pdf', size: pdfBuffer.length, url: uploadResult.url, cloudinaryId: uploadResult.storageId, documentType: 'shipping_doc', portalVisible: true });
    console.log('[ShipmentDoc] Saved ' + filename);
  } catch (e) { console.error('[ShipmentDoc] Failed:', e.message); }
}
...
function cleanNumericFields(data, fields) {
  ...
  // A part ALWAYS has a quantity — never null (a blank box used to save as null and come back empty).
    ...
    try {
      ...
      sortedParts.forEach((partObj, index) => {
        ...
        if (partObj.partType === 'cone_roll') {
          ...
          const ldType = coneSpecLabel(partObj._coneLargeDiaType, partObj._coneLargeDiaMeasure);
          const sdType = coneSpecLabel(partObj._coneSmallDiaType, partObj._coneSmallDiaMeasure);
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (search) {
      ...
      if (!isNaN(drParsed)) {
        searchMatches.push({ drNumber: drParsed });
      }

      // A VOIDED order should only surface when you go looking for it specifically — by DR number
      // or by the client's PO. Searching a client name shouldn't dredge up voided duplicates.
      const voidedOnlyIfDirect = [
        { isVoided: { [Op.not]: true } },
        { clientPurchaseOrderNumber: { [Op.iLike]: searchLower } }
      ];
      if (!isNaN(drParsed)) voidedOnlyIfDirect.push({ drNumber: drParsed });

      where[Op.and] = [
        { [Op.or]: searchMatches },
        { [Op.or]: voidedOnlyIfDirect }
      ];
    } else {
    ...
    if (archived === 'true') {
      ...
      // Voided orders keep their old status (processing, waiting_for_materials...), so a status
      // filter alone never removed them — they kept showing on the schedule and in inventory.
      where.isVoided = { [Op.not]: true };
    }
...
router.get('/invoicing/queue', async (req, res, next) => {
  try {
    ...
    const workOrders = await WorkOrder.findAll({
      where: {
        [Op.or]: [{ isVoided: null }, { isVoided: false }], // don't invoice a voided order
...
router.post('/', async (req, res, next) => {
  try {
    ...
    try {
      ...
      let drNumber = null;
      if (customDRNumber) {
        // Use custom DR number — do NOT advance the sequence. Availability is checked under
        // the same advisory lock the auto-allocator uses, so the number cannot be taken
        ...
        // Allocated under an advisory lock so two simultaneous creates cannot claim the same
        // number. Also skips past the configured next_dr_number if it has gone stale, instead
        // of handing out a duplicate and dying on the unique index.
        drNumber = await allocateDRNumber(require('../models'), transaction);
...
router.put('/:id/status', async (req, res, next) => {
  try {
    ...
    // Job finished → refresh Ginger's list (free, no-AI) so it drops off and capacity recalcs
    if (['completed', 'stored', 'shipped', 'archived'].includes(status)) {
      require('../services/gingerScan').runGingerScan({ useAI: false }).catch(() => {});
    }
...
router.post('/:id/mark-complete', async (req, res, next) => {
  try {
    ...
    // Mark complete. Completion time is recorded on the order (completedAt) and per part —
    // no longer appended to notes (keeps the notes field for real notes).
    const now = new Date();
    ...
    setImmediate(async () => {
      try {
        ...
        // Never auto-generate a certificate whose origin is unproven. Auto-generation does not
        // pass acknowledgeOriginIssues, so the endpoint would refuse anyway — checking here
        // turns that into a clear log line and an alert flag instead of a swallowed 409.
        const { checkOrigin } = require('../services/materialOrigin');
        const certParts = (freshWO.parts || []).filter(p => !['fab_service','shop_rate','rush_service','inspection'].includes(p.partType));
        const originCheck = checkOrigin(certParts);
        if (!originCheck.ok) {
          console.warn('[USMCA] Auto-generate SKIPPED for WO', freshWO.drNumber || freshWO.orderNumber,
            '— origin not established:', originCheck.issues.map(i => i.detail).join(' | '));
          try {
            await freshWO.update({ usmcaBlockedReason: originCheck.issues.map(i => i.detail).join('\n') });
          } catch (_) { /* column is advisory only */ }
          return;
        }
...
router.post('/:id/pickup', async (req, res, next) => {
  try {
    const { type, pickedUpBy, items, signature } = req.body; // signature: base64 PNG data URL (optional)
...
// GET /blank-shipper - Generate a blank shipper form PDF
router.get('/blank-shipper', async (req, res, next) => {
  try {
    const PDFDocument = require('pdfkit');
    const doc = new PDFDocument({ margin: 50, size: 'letter' });
    const chunks = [];
    ...
    // Header — same style as pickup receipt
    if (logoFile) try { doc.image(logoFile, 50, 20, { width: 60 }); } catch {}
    if (hasYellowcake) doc.font('Yellowcake').fontSize(14).fillColor('#333').text('Carolina Rolling Co. Inc.', 125, 28);
    else doc.font('Helvetica-Bold').fontSize(14).fillColor('#333').text('CAROLINA ROLLING CO. INC.', 125, 28);
    ...
    // Title + Shipper # box (top right)
    doc.font('Helvetica-Bold').fontSize(11).fillColor('#e65100').text('SHIPPER', 50, 102);
    // Shipper # box
    doc.font('Helvetica').fontSize(8).fillColor('#666').text('Shipper #:', 390, 98);
    doc.rect(430, 94, 130, 18).lineWidth(0.8).strokeColor('#999').stroke();
    doc.moveTo(50, 128).lineTo(562, 128).lineWidth(0.5).strokeColor('#e0e0e0').stroke();

    // Info row — blank fields matching receipt style
    let y = 138;
    const fieldLine = (label, x, yy, w) => {
      doc.font('Helvetica').fontSize(8).fillColor('#666').text(label, x, yy);
      doc.moveTo(x + doc.widthOfString(label) + 4, yy + 9).lineTo(x + w, yy + 9).lineWidth(0.5).strokeColor('#bbb').stroke();
    };

    doc.font('Helvetica').fontSize(8).fillColor('#666').text('Customer', 50, y); y += 12;
    doc.moveTo(50, y + 9).lineTo(300, y + 9).lineWidth(0.5).strokeColor('#bbb').stroke(); y += 20;
    fieldLine('P.O:', 50, y, 200); 
    fieldLine('Date:', 310, y, 200); y += 20;
    fieldLine('Picked Up By:', 50, y, 300); y += 20;
    fieldLine('Driver / Carrier:', 50, y, 300); 
    fieldLine('Vehicle:', 310, y, 200); y += 28;

    doc.moveTo(50, y).lineTo(562, y).lineWidth(0.5).strokeColor('#e0e0e0').stroke(); y += 12;

    // Items table header
    doc.rect(50, y, 512, 16).fill('#1a1a1a');
    doc.font('Helvetica-Bold').fontSize(8).fillColor('white');
    doc.text('QTY', 55, y + 4, { width: 50, lineBreak: false });
    doc.text('PART #', 110, y + 4, { width: 80, lineBreak: false });
    doc.text('DESCRIPTION', 195, y + 4, { width: 370 });
    y += 16;
    ...
    for (let i = 0; i < numLines; i++) {
      if (i % 2 === 1) doc.rect(50, y, 512, lineHeight).fill('#f9f9f9');
      doc.rect(50, y, 512, lineHeight).lineWidth(0.3).strokeColor('#ddd').stroke();
      // Subtle column dividers
      doc.moveTo(105, y).lineTo(105, y + lineHeight).lineWidth(0.3).strokeColor('#ddd').stroke();
      doc.moveTo(190, y).lineTo(190, y + lineHeight).lineWidth(0.3).strokeColor('#ddd').stroke();
      y += lineHeight;
    }
    ...
    // Total row
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#333').text('Total Items:', 55, y, { lineBreak: false });
    doc.moveTo(120, y + 10).lineTo(200, y + 10).lineWidth(0.5).strokeColor('#bbb').stroke();
    y += 28;

    // Notes field
    doc.font('Helvetica').fontSize(8).fillColor('#666').text('Notes / Special Instructions:', 55, y); y += 14;
    doc.rect(50, y, 512, 48).lineWidth(0.5).strokeColor('#bbb').stroke(); y += 60;

    // Signature block
    doc.moveTo(50, y).lineTo(562, y).lineWidth(0.5).strokeColor('#e0e0e0').stroke(); y += 16;
    doc.font('Helvetica').fontSize(8).fillColor('#666');
    doc.moveTo(50, y + 18).lineTo(250, y + 18).lineWidth(0.5).strokeColor('#555').stroke();
    doc.text('Received By / Signature', 50, y + 22);
    doc.moveTo(310, y + 18).lineTo(510, y + 18).lineWidth(0.5).strokeColor('#555').stroke();
    doc.text('Date', 310, y + 22);
    ...
    res.set('Content-Disposition', 'inline; filename="CR-Blank-Shipper.pdf"');
    res.send(pdfBuffer);
  } catch (error) {
    next(error);
  }
...
router.put('/:id/pickup/:index', async (req, res, next) => {
  try {
    ...
    // Regenerate shipment doc in background
    setImmediate(async () => {
      try {
        const freshWO = await WorkOrder.findByPk(workOrder.id, { include: [{ model: WorkOrderPart, as: 'parts' }] });
        if (freshWO) await saveShipmentDocument(freshWO, freshWO.pickupHistory[idx], idx);
      } catch (e) { console.error('[ShipmentDoc] update hook error:', e.message); }
    });
  } catch (error) { next(error); }
});
...
router.delete('/:id', async (req, res, next) => {
  ...
  try {
    ...
    // Snapshot everything before it goes. The order was loaded with its parts, part files and
    // documents, so the archived JSON is enough to reconstruct it if this delete turns out to
    // have been a mistake. Archiving never blocks the delete — see services/deletionArchive.js.
    ...
    await archiveRecord(workOrder, {
      ...
      reason: req.body?.reason || (workOrder.drNumber ? 'force-delete override' : null),
      transaction
    });
...
router.post('/:id/parts', async (req, res, next) => {
  try {
    ...
    // Auto-fill lot number as <DR number>-<clean part number> when not provided.
    // The clean number ignores service line-items so production parts read 1, 2, 3...
    let cleanLotNum = partNumber;
    try {
      const existingForLot = await WorkOrderPart.findAll({
        where: { workOrderId: workOrder.id },
        attributes: ['id', 'partNumber', 'partType', 'formData']
      });
      const synthetic = existingForLot.map(p => ({ id: p.id, partNumber: p.partNumber, partType: p.partType, formData: p.formData }));
      synthetic.push({ id: '__new__', partNumber, partType, formData: formDataJson, _linkedPartId: req.body._linkedPartId });
      const { display } = computeDisplayNumbers(synthetic);
      cleanLotNum = display['__new__'] || partNumber;
    } catch (e) { cleanLotNum = partNumber; }

    const autoLotNumber = (req.body.lotNumber && String(req.body.lotNumber).trim())
      ? req.body.lotNumber
      : (workOrder.drNumber ? `${workOrder.drNumber}-${cleanLotNum}` : (req.body.lotNumber || null));
...
router.put('/:id/parts/:partId', async (req, res, next) => {
  try {
    ...
    // Extract underscore-prefixed fields into formData JSONB.
    // Merged over the stored formData rather than replacing it — a partial update that omits
    // some `_` fields must not erase the part's saved geometry. Explicitly sending a key as
    // '' or null still clears it.
    const extractedUpdate = extractFormData(req.body);
    let formDataJson = extractedUpdate.formData || null;
    if (formDataJson && part && part.formData && typeof part.formData === 'object') {
      formDataJson = { ...part.formData, ...formDataJson };
    }
    ...
    // Auto-fill lot number (<DR>-<line>) if it would still be empty after this update
    const effLot = updates.lotNumber !== undefined ? updates.lotNumber : part.lotNumber;
    if (!effLot || !String(effLot).trim()) {
      const woForLot = await WorkOrder.findByPk(req.params.id);
      if (woForLot?.drNumber) {
        let cleanNum = part.partNumber;
        try {
          const allForLot = await WorkOrderPart.findAll({ where: { workOrderId: req.params.id }, attributes: ['id', 'partNumber', 'partType', 'formData'] });
          const { display } = computeDisplayNumbers(allForLot.map(p => ({ id: p.id, partNumber: p.partNumber, partType: p.partType, formData: p.formData })));
          cleanNum = display[part.id] || part.partNumber;
        } catch (e) { cleanNum = part.partNumber; }
        updates.lotNumber = `${woForLot.drNumber}-${cleanNum}`;
      }
    }
      ...
      try {
        ...
        for (const svc of linkedServices) {
          if (svc.status !== 'completed') {
            await svc.update({ status: 'completed', completedAt: new Date(), completedBy: part.completedBy || svc.completedBy || null });
        ...
        if (allRegularDone) {
          ...
          for (const rush of rushParts) {
            await rush.update({ status: 'completed', completedAt: new Date(), completedBy: part.completedBy || rush.completedBy || null });
          ...
          setImmediate(async () => {
            try {
              ...
              const fileStorage = require('../utils/storage');
              const cocParts = (wo.parts || []).filter(p => !['fab_service', 'shop_rate', 'rush_service', 'inspection'].includes(p.partType));
              ...
              const pdfBuffer = await buildCocPdfBuffer(wo, { parts: cocParts, wps: null, certifiedBy: 'Jason Thornton', dateStr });
    ...
    for (const file of req.files) {
      try {
        ...
        const document = await WorkOrderDocument.create({
          ...
          portalVisible: req.body.portalVisible === 'true' || req.body.portalVisible === true
            || ['mtr', 'coc', 'shipping_doc'].includes(req.body.documentType) || false
        });
...
// POST /api/workorders/wps/render-pdf - Render arbitrary HTML into a clean PDF (no browser chrome). Used by the WPS generator.
// Body: { html: string, filename?: string }
router.post('/wps/render-pdf', async (req, res, next) => {
  try {
    const puppeteer = require('puppeteer-core');
    const { execSync } = require('child_process');
    let html = req.body.html || '';
    const filename = String(req.body.filename || 'document').replace(/[^a-zA-Z0-9._-]/g, '_');
    if (!html) return res.status(400).json({ error: { message: 'html required' } });

    let chromePath = (process.env.PUPPETEER_EXECUTABLE_PATH && fs.existsSync(process.env.PUPPETEER_EXECUTABLE_PATH)) ? process.env.PUPPETEER_EXECUTABLE_PATH : null;
    if (!chromePath) {
      const candidates = ['/app/.chrome-for-testing/chrome-linux64/chrome', '/app/.chrome-for-testing/chrome-linux/chrome', '/app/.apt/usr/bin/google-chrome', '/usr/bin/google-chrome', '/usr/bin/chromium'];
      for (const p of candidates) { if (fs.existsSync(p)) { chromePath = p; break; } }
      if (!chromePath) { try { const found = execSync('which chrome google-chrome chromium 2>/dev/null || find /app -name "chrome" -type f 2>/dev/null | head -1').toString().trim(); if (found) chromePath = found.split('\n')[0]; } catch (e) {} }
    }
    if (!chromePath) return res.status(500).json({ error: { message: 'PDF renderer unavailable' } });

    // Embed logo + font so they render inside Chrome
    try {
      const logoPath = [path.join(__dirname, '..', 'assets', 'logo.png'), path.join(__dirname, '..', 'assets', 'logo.jpg')].find(p => fs.existsSync(p));
      if (logoPath && html.includes('/logo.png')) { const mime = logoPath.endsWith('.jpg') ? 'image/jpeg' : 'image/png'; html = html.split('/logo.png').join(`data:${mime};base64,${fs.readFileSync(logoPath).toString('base64')}`); }
    } catch (e) {}
    try {
      const fontPath = path.join(__dirname, '..', 'assets', 'fonts', 'Yellowcake-Regular.ttf');
      if (fs.existsSync(fontPath) && html.includes('/fonts/Yellowcake-Regular.ttf')) { html = html.split('/fonts/Yellowcake-Regular.ttf').join(`data:font/ttf;base64,${fs.readFileSync(fontPath).toString('base64')}`); }
    } catch (e) {}

    const browser = await puppeteer.launch({ executablePath: chromePath, headless: 'new', args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-gpu'] });
    try {
      const page = await browser.newPage();
      await page.setContent(html, { waitUntil: 'load', timeout: 15000 });
      const pdf = await page.pdf({ format: 'Letter', margin: { top: '0.4in', bottom: '0.4in', left: '0.4in', right: '0.4in' }, printBackground: true });
...
router.post('/:id/print-package', async (req, res, next) => {
  try {
    ...
    if (workOrderHtml) {
      try {
        ...
        if (chromePath) {
          ...
          // Embed the company logo (used in the work-order header) so Puppeteer can render it
          try {
            const logoPath = [path.join(__dirname, '..', 'assets', 'logo.png'), path.join(__dirname, '..', 'assets', 'logo.jpg')].find(p => fs.existsSync(p));
            if (logoPath && processedHtml.includes('/logo.png')) {
              const mime = logoPath.endsWith('.jpg') ? 'image/jpeg' : 'image/png';
              const logoUri = `data:${mime};base64,${fs.readFileSync(logoPath).toString('base64')}`;
              processedHtml = processedHtml.split('/logo.png').join(logoUri);
            }
          } catch (e) { console.warn('[print-package] Could not embed logo:', e.message); }

          // Embed the Yellowcake font so the company name renders in the brand font
          try {
            const fontPath = path.join(__dirname, '..', 'assets', 'fonts', 'Yellowcake-Regular.ttf');
            if (fs.existsSync(fontPath) && processedHtml.includes('/fonts/Yellowcake-Regular.ttf')) {
              const fontUri = `data:font/ttf;base64,${fs.readFileSync(fontPath).toString('base64')}`;
              processedHtml = processedHtml.split('/fonts/Yellowcake-Regular.ttf').join(fontUri);
            }
          } catch (e) { console.warn('[print-package] Could not embed font:', e.message); }
    ...
    const pdfSources = [];
    // Optional per-section selection from the print dialog. When absent, keep the legacy mode behavior.
    const opt = req.body.options || null;
    const docCategory = (dt) => {
      if (dt === 'inspection_report') return 'inspectionReports';
      if (dt === 'shipping_doc' || dt === 'pickup_receipt') return 'shippingDocs';
      if (dt === 'coc' || dt === 'usmca' || dt === 'mtr') return 'certs';
      if (dt === 'purchase_order' || dt === 'outside_processing_po') return 'pos';
      return 'otherDocs';
    };

    // Part PDFs (the prints/drawings)
    if (!opt || opt.prints) {
    ...
    if (workOrder.documents) {
      for (const doc of workOrder.documents) {
        if (!(doc.mimeType === 'application/pdf' || (doc.originalName || '').toLowerCase().endsWith('.pdf'))) continue;
        const dt = doc.documentType;
        let include;
        if (opt) {
          include = !!opt[docCategory(dt)];
        } else {
          // Legacy: everything except MTR; POs only in full mode
          if (dt === 'mtr') include = false;
          else if (dt === 'purchase_order') include = (mode === 'full');
          else include = true;
        }
        if (include) {
          pdfSources.push({ label: `Doc: ${doc.originalName}`, proxyUrl: `${baseUrl}/documents/${doc.id}/download` });
        }
      }
    }
...
// ===== SHARED COC PDF BUILDER — used by BOTH the manual /:id/coc route and the auto-COC generator =====
async function buildCocPdfBuffer(workOrder, { parts, wps, certifiedBy, dateStr }) {
  const PDFDocument = require('pdfkit');
  const doc = new PDFDocument({ margin: 50, size: 'letter' });
  const chunks = [];
  doc.on('data', c => chunks.push(c));
  ...
  doc.text('Customer', 50, y); y += 14;
  doc.font('Helvetica').fontSize(10).fillColor(darkColor);
  doc.text(workOrder.clientName || '', 50, y); y += 14;
  ...
  // Parts table header
  doc.moveTo(50, y).lineTo(562, y).lineWidth(0.5).strokeColor('#e0e0e0').stroke(); y += 8;
  ...
  y += 12;
  doc.moveTo(50, y).lineTo(562, y).lineWidth(0.3).strokeColor('#ddd').stroke(); y += 7;

  // Elongation + material helpers
  const parseInches = (s) => {
    if (s === null || s === undefined) return 0;
    let str = String(s).replace(/["″]|in\.?|inch(es)?/gi, ' ').trim();
    let m = str.match(/(\d+)[\s-]+(\d+)\s*\/\s*(\d+)/); if (m) return parseInt(m[1]) + parseInt(m[2]) / parseInt(m[3]);
    m = str.match(/(\d+)\s*\/\s*(\d+)/); if (m) return parseInt(m[1]) / parseInt(m[2]);
    m = str.match(/(\d*\.?\d+)/); if (m) return parseFloat(m[1]);
    return 0;
  };
  const isStainlessMat = (s) => /s\s*\/\s*s|stainless|\b3[0-4][0-9]\b|\b316\b|\b304\b/i.test(String(s || ''));
  const isAluminumMat = (s) => /alum|\b(6061|5052|5083|3003|6063|2024|7075|5454|5754|1100)\b/i.test(String(s || ''));
  const fiberElongation = (t, id) => (t > 0 && id > 0) ? (50 * t / ((id + t) / 2)) : 0;
  const formingClauses = new Set();
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (rollDesc) {
      const cleanRollDesc = rollDesc.replace(/°/g, 'deg').replace(/↻/g, '(CW)').replace(/↺/g, '(CCW)');
      doc.font('Helvetica').fontSize(10.5).fillColor(grayColor);
    ...
    if (trackingBits.length) {
      const trackingLine = trackingBits.join('   ');
      doc.font('Helvetica').fontSize(9.5).fillColor('#555');
      doc.text(trackingLine, 200, y, { width: 360 });
      y += doc.heightOfString(trackingLine, { width: 360 }) + 1;
    }

    // Extreme fiber elongation (UG-79) — ONLY valid for plate rolled into a curved shell (plate/shaped plate/cone).
    // Structural shapes (angle/channel/beam/tee/bar), pipe/tube, and press-brake use different geometry/formulas
    // and are usually not pressure parts, so no UG-79 elongation note is printed for them.
    const elongationApplies = ['plate_roll', 'shaped_plate', 'cone_roll'].includes(p.partType);
    if (elongationApplies) {
      const tIn = parseInches(p.thickness || fd.thickness || p.wallThickness || fd.wallThickness);
      const idIn = parseInches(p.innerDiameter || fd.innerDiameter || p.diameter || fd.diameter);
      const matField = p.material || fd.material || matDesc;
      const aluminum = isAluminumMat(matField);
      const clause = aluminum ? 'UNF-79' : (isStainlessMat(matField) ? 'UHA-44' : 'UCS-79');
      formingClauses.add(clause);
      const efe = fiberElongation(tIn, idIn);
      if (efe > 0) {
        const efeStr = efe.toFixed(1);
        let note, ok;
        if (aluminum) {
          note = `Extreme fiber elongation ${efeStr}% — evaluate ${clause} for this aluminum alloy/temper (post-forming heat treatment is alloy/temper-specific).`;
          ok = false;
        } else if (efe <= 5) {
          note = `Extreme fiber elongation ${efeStr}% (<= 5%) — post-forming heat treatment not required per ${clause}.`;
          ok = true;
        } else {
          note = `Extreme fiber elongation ${efeStr}% (> 5%) — evaluate ${clause} for post-forming heat treatment.`;
          ok = false;
        }
        doc.font('Helvetica-Oblique').fontSize(9).fillColor(ok ? '#2e7d32' : '#c62828');
        doc.text(note, 200, y, { width: 360 });
        y += doc.heightOfString(note, { width: 360 }) + 1;
      }
    }
    y += 4;
    if (i < parts.length - 1) { doc.moveTo(85, y).lineTo(562, y).lineWidth(0.3).strokeColor('#eee').stroke(); y += 4; }
  }
  ...
  doc.moveTo(50, y).lineTo(562, y).lineWidth(0.5).strokeColor('#e0e0e0').stroke(); y += 12;
  doc.font('Helvetica').fontSize(11).fillColor(darkColor);
  doc.text('We hereby certify that parts described above were cold formed and comply with', 50, y, { width: 512, align: 'center' }); y += 13;
  doc.font('Helvetica-Bold').fontSize(10).fillColor(darkColor);
  const calloutList = ['UG-79', 'UG-80', ...Array.from(formingClauses)];
  doc.text('ASME Section VIII Div.1, ' + calloutList.join(', '), 50, y, { width: 512, align: 'center' }); y += 26;
...
router.post('/:id/coc', async (req, res, next) => {
  try {
    ...
    const allParts = (workOrder.parts || []).filter(p => !['fab_service', 'shop_rate', 'rush_service', 'inspection'].includes(p.partType));
    const { selectedPartIds } = req.body;
    const parts = selectedPartIds && selectedPartIds.length > 0 ? allParts.filter(p => selectedPartIds.includes(p.id)) : allParts;
    const pdfBuffer = await buildCocPdfBuffer(workOrder, { parts, wps, certifiedBy, dateStr });
...
router.post('/:id/usmca', async (req, res, next) => {
  try {
    ...
    // ---- Country of origin -------------------------------------------------------------
    // Rolling a bar into a ring does not shift the tariff heading or substantially transform
    // the material, so the ring's origin IS the material's origin. It is read from the heat
    // records rather than assumed, and a part whose heats came from different countries
    // produces one certificate line per country.
    const { checkOrigin, buildOriginLineItems } = require('../services/materialOrigin');
    const originCheck = checkOrigin(parts);

    if (!originCheck.ok && req.body.acknowledgeOriginIssues !== true) {
      // Refuse by default. This document is signed under a statement of liability, so
      // generating one from incomplete or non-originating material should be a deliberate act,
      // not the path of least resistance. The caller can override explicitly.
      return res.status(409).json({
        error: {
          message: 'Cannot certify: material origin is incomplete or not USMCA-originating.',
          code: 'ORIGIN_NOT_ESTABLISHED',
          issues: originCheck.issues,
          hint: 'Record the country of origin on each heat, or resend with acknowledgeOriginIssues to override.'
        }
      });
    }
    ...
    if (req.body.lineItems && req.body.lineItems.length > 0) {
      partDescriptions = req.body.lineItems.map(li => ({
        ...
        originCountry: li.originCountry || ''
      }));
    } else {
      partDescriptions = buildOriginLineItems(parts, { goodsDescription, htsCode });
    }

    if (!originCheck.ok) {
      console.warn('[USMCA] Generated with acknowledged origin issues for WO',
        workOrder.drNumber || workOrder.orderNumber,
        originCheck.issues.map(i => i.type + ': ' + i.detail));
    }

    // Origin column text. Falls back to the certificate-level country only if a line has no
    // recorded origin — and that path is only reachable when the caller overrode the check.
    const originLabel = (line) => line.originCountry || req.body.defaultOriginCountry || '';
    ...
    if (format === 'format2') {
      ...
      y+=r1h;
      // Row2
      // Calculate row2 height based on importer address content
      const impLines2=(importerAddress||'').split('\n').filter(l=>l.trim()).length;
      const impNameH2=importerName?11:0;
      const r2h=Math.max(62, 20 + impNameH2 + impLines2*11 + 16);
      ...
      doc.font('Helvetica').fontSize(7).fillColor(gry).text('TAX IDENTIFICATION NUMBER:',43,y+r2h-12);
      ...
      let iy=y+15;
      if (importerName) { doc.font('Helvetica').fontSize(9).fillColor(blk).text(importerName,43+half,iy,{width:half-10}); iy+=12; }
      (importerAddress||'').split('\n').forEach(l=>{if(l.trim()){doc.font('Helvetica').fontSize(8.5).fillColor(blk).text(l.trim(),43+half,iy,{width:half-10});iy+=11;}});
      doc.font('Helvetica').fontSize(7).fillColor(gry).text('TAX IDENTIFICATION NUMBER:',43+half,y+r2h-12);
      y+=r2h;
      // Table header
      ...
      partDescriptions.forEach(p=>{
        ...
        const rv=[p.partNum||'',p.description,pHts,originCriteria,originLabel(p)];
        cx.forEach((x,i)=>{
      ...
      y+=65;
            // Dynamic row height for importer address
      const impLines1=(importerAddress||'').split('\n').filter(l=>l.trim()).length;
      const impNameH1=importerName?12:0;
      const f1r2h=Math.max(58, 20 + impNameH1 + impLines1*11 + 16);
      doc.rect(40,y,half,f1r2h).lineWidth(0.5).strokeColor(blk).stroke();
      ...
      doc.font('Helvetica').fontSize(7.5).fillColor(gry).text('Tax Identification Number: (Tax ID)',43,y+f1r2h-12);
      doc.rect(40+half,y,half,f1r2h).lineWidth(0.5).strokeColor(blk).stroke();
      ...
      let iy2=y+14;
      if(importerName){doc.font('Helvetica').fontSize(9).fillColor(blk).text(importerName,43+half,iy2,{width:half-10});iy2+=12;}
      (importerAddress||'').split('\n').forEach(l=>{if(l.trim()){doc.font('Helvetica').fontSize(8.5).fillColor(blk).text(l.trim(),43+half,iy2,{width:half-10});iy2+=11;}});
      doc.font('Helvetica').fontSize(7.5).fillColor(gry).text('Registro Federal de Contribuyentes: (RFC)',43+half,y+f1r2h-12);
      y+=f1r2h;
      ...
      partDescriptions.forEach((p,idx)=>{
        ...
        const ph=p.htsCode||htsCode; [ph,originCriteria,originLabel(p)].forEach((v,i)=>{doc.rect(cx2[i+2],y,cw2[i+2],rh).lineWidth(0.5).strokeColor(blk).stroke();doc.font('Helvetica').fontSize(9).fillColor(blk).text(v,cx2[i+2]+3,y+6,{width:cw2[i+2]-6,align:'center',lineBreak:false});
        });
        y+=rh;
      });
      y+=6;
...
// ── ShipmentCharge CRUD for Work Orders ──
router.get('/:id/shipment-charges', async (req, res, next) => {
  try {
    ...
    // If vendor JOIN fails due to type mismatch, retry without the JOIN
...
router.post('/:id/shipment-charges', async (req, res, next) => {
  try {
    ...
    const maxOrder = await ShipmentCharge.max('sortOrder', { where: { workOrderId: req.params.id } }) || 0;
...
router.put('/:id/shipment-charges/:chargeId', async (req, res, next) => {
  try {
    const charge = await ShipmentCharge.findOne({ where: { id: req.params.chargeId, workOrderId: req.params.id } });
...
// PATCH /:id/usmca-info — Save per-order USMCA importer/HTS info
router.patch('/:id/usmca-info', async (req, res, next) => {
  try {
    const wo = await WorkOrder.findByPk(req.params.id);
    if (!wo) return res.status(404).json({ error: { message: 'Work order not found' } });
    const { usmcaImporterName, usmcaImporterAddress, usmcaHtsCode, usmcaOriginCriteria } = req.body;
    await wo.update({
      ...(usmcaImporterName !== undefined && { usmcaImporterName: usmcaImporterName || null }),
      ...(usmcaImporterAddress !== undefined && { usmcaImporterAddress: usmcaImporterAddress || null }),
      ...(usmcaHtsCode !== undefined && { usmcaHtsCode: usmcaHtsCode || null }),
      ...(usmcaOriginCriteria !== undefined && { usmcaOriginCriteria: usmcaOriginCriteria || null }),
    });
    res.json({ data: wo, message: 'USMCA info saved' });
  } catch (error) { next(error); }
});

// ===== Per-work-order operator <-> office messaging =====
function messageSender(req) {
  if (req.user) {
    const sig = (req.user.signatureName || '').trim();
    const first = sig ? sig.split(/\s+/)[0] : (req.user.username || 'Office');
    return { senderType: 'office', senderName: first };
  }
  return { senderType: 'operator', senderName: (req.operatorName || (req.apiKey && req.apiKey.name) || 'Operator') };
}

// Office unread badge — operator messages the office hasn't seen yet (define before /:id/messages)
router.get('/messages/unread-count', async (req, res, next) => {
  try {
    const { WorkOrderMessage } = require('../models');
    const count = await WorkOrderMessage.count({ where: { senderType: 'operator', readByOffice: false } });
    res.json({ data: { count } });
  } catch (error) { next(error); }
});

// Alert list — which work orders have unread messages for the requesting side (office sees unread
// operator messages; operator sees unread office messages). Most recent + count each.
router.get('/messages/unread', async (req, res, next) => {
  try {
    const { WorkOrderMessage, WorkOrder } = require('../models');
    const { Op } = require('sequelize');
    const isOffice = !!req.user;
    if (isOffice) {
      // Office: unread operator messages across all work orders
      const unread = await WorkOrderMessage.findAll({
        where: { senderType: 'operator', readByOffice: false },
        order: [['createdAt', 'DESC']],
        include: [{ model: WorkOrder, as: 'workOrder', attributes: ['id', 'drNumber', 'clientName'] }]
      });
      const byWo = {};
      for (const m of unread) {
        const wo = m.workOrder; if (!wo) continue;
        if (!byWo[wo.id]) byWo[wo.id] = { workOrderId: wo.id, drNumber: wo.drNumber, clientName: wo.clientName, senderName: m.senderName, body: m.body, hasImage: !!m.imageUrl, createdAt: m.createdAt, count: 0 };
        byWo[wo.id].count += 1;
      }
      return res.json({ data: Object.values(byWo), total: unread.length });
    }
    // Operator: only office messages on work orders assigned to THIS operator, not yet read by them
    const operatorName = req.operatorName;
    if (!operatorName) return res.json({ data: [], total: 0 });
    const myWos = await WorkOrder.findAll({ where: { assignedOperator: operatorName }, attributes: ['id', 'drNumber', 'clientName'] });
    if (!myWos.length) return res.json({ data: [], total: 0 });
    const woMap = {}; myWos.forEach(w => { woMap[w.id] = w; });
    const msgs = await WorkOrderMessage.findAll({
      where: { senderType: 'office', workOrderId: { [Op.in]: myWos.map(w => w.id) } },
      order: [['createdAt', 'DESC']]
    });
    const byWo = {};
    let total = 0;
    for (const m of msgs) {
      if ((m.readByOperators || []).includes(operatorName)) continue;
      const wo = woMap[m.workOrderId]; if (!wo) continue;
      if (!byWo[wo.id]) byWo[wo.id] = { workOrderId: wo.id, drNumber: wo.drNumber, clientName: wo.clientName, senderName: m.senderName, body: m.body, hasImage: !!m.imageUrl, createdAt: m.createdAt, count: 0 };
      byWo[wo.id].count += 1; total += 1;
    }
    res.json({ data: Object.values(byWo), total });
  } catch (error) { next(error); }
});

// Unread count for THIS work order, for the requesting side (does NOT mark anything read)
router.get('/:id/messages/unread-count', async (req, res, next) => {
  try {
    const { WorkOrderMessage } = require('../models');
    const isOffice = !!req.user;
    if (isOffice) {
      const count = await WorkOrderMessage.count({ where: { workOrderId: req.params.id, senderType: 'operator', readByOffice: false } });
      return res.json({ data: { count } });
    }
    const operatorName = req.operatorName;
    const msgs = await WorkOrderMessage.findAll({ where: { workOrderId: req.params.id, senderType: 'office' }, attributes: ['readByOperators'] });
    const count = operatorName ? msgs.filter(m => !(m.readByOperators || []).includes(operatorName)).length : 0;
    res.json({ data: { count } });
  } catch (error) { next(error); }
});

// Mark this work order's incoming messages read for the requesting side (used by "Clear")
router.post('/:id/messages/mark-read', async (req, res, next) => {
  try {
    const { WorkOrderMessage } = require('../models');
    const isOffice = !!req.user;
    if (isOffice) {
      await WorkOrderMessage.update({ readByOffice: true }, { where: { workOrderId: req.params.id, senderType: 'operator', readByOffice: false } });
    } else {
      const operatorName = req.operatorName;
      if (operatorName) {
        const msgs = await WorkOrderMessage.findAll({ where: { workOrderId: req.params.id, senderType: 'office' } });
        for (const m of msgs) {
          const readers = m.readByOperators || [];
          if (!readers.includes(operatorName)) await m.update({ readByOperators: [...readers, operatorName], readByOperator: true });
        }
      }
    }
...
// List the thread for a work order (and mark the viewer's side caught up)
router.get('/:id/messages', async (req, res, next) => {
  try {
    const { WorkOrderMessage } = require('../models');
    const isOffice = !!req.user;
    const msgs = await WorkOrderMessage.findAll({ where: { workOrderId: req.params.id }, order: [['createdAt', 'ASC']] });
    if (isOffice) {
      await WorkOrderMessage.update({ readByOffice: true }, { where: { workOrderId: req.params.id, senderType: 'operator', readByOffice: false } });
    } else if (req.operatorName) {
      const operatorName = req.operatorName;
      for (const m of msgs) {
        if (m.senderType !== 'office') continue;
        const readers = m.readByOperators || [];
        if (!readers.includes(operatorName)) await m.update({ readByOperators: [...readers, operatorName], readByOperator: true });
      }
    }
    res.json({ data: msgs.map(m => m.toJSON()), total: msgs.length });
  } catch (error) { next(error); }
});

// Send a message (text and/or a photo as a base64 data URL)
router.post('/:id/messages', async (req, res, next) => {
  try {
    const { WorkOrderMessage } = require('../models');
    const { body, image } = req.body;
    if ((!body || !String(body).trim()) && !image) return res.status(400).json({ error: { message: 'Message is empty' } });
    const { senderType, senderName } = messageSender(req);
    let imageUrl = null, imageStorageId = null;
    if (typeof image === 'string' && image.startsWith('data:image')) {
      try {
        ...
        if (m) {
          ...
          const up = await fileStorage.uploadBuffer(buf, { folder: 'work-orders/' + req.params.id + '/messages', filename: 'msg-' + Date.now() + '.' + ext, mimeType: m[1] });
          imageUrl = up.url; imageStorageId = up.storageId;
        }
      } catch (e) { console.error('[Messages] image upload failed:', e.message); }
    }
    const msg = await WorkOrderMessage.create({
      workOrderId: req.params.id, senderType, senderName,
      body: body ? String(body).trim() : null, imageUrl, imageStorageId,
      readByOffice: senderType === 'office', readByOperator: senderType === 'operator'
    });
