...
app.get('/api/debug/models', async (req, res) => {
  ...
  try {
    ...
    if (!apiKey) return res.json({ version: 'v291', keyPresent: false, reason: 'ANTHROPIC_API_KEY is NOT set on this server' });
  ...
  } catch (e) { res.json({ version: 'v291', error: e.message }); }
});

// GET /api/version - no auth; hit this in a browser to confirm which backend build is actually running
app.get('/api/version', (req, res) => {
  res.set('Cache-Control', 'no-store');
  res.json({ version: 'v291', built: '2026-06-13', note: 'v248 — manual AI parse runs in background (fixes 30s timeout).' });
});
