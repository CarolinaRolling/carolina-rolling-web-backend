...
// GET /api/debug/estimator-keys - no auth; shows which API keys unlock the Quotes tab
app.get('/api/debug/estimator-keys', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  try {
    const { ApiKey } = require('./models');
    const keys = await ApiKey.findAll({ attributes: ['name', 'operatorName', 'permissions', 'isEstimator', 'isActive'] });
    const rows = keys.map(k => ({
      name: k.name,
      operatorName: k.operatorName,
      permissions: k.permissions,
      isEstimator: k.isEstimator,
      isActive: k.isActive,
    ...
    res.json({
      ...
      hint: rows.some(r => r.unlocksQuotesTab)
        ? 'Use one of the "qualifying" keys in the phone app to see the Quotes tab.'
        : '❌ No API key qualifies. Create one with permissions=admin (or isEstimator=true) and enter it in the phone app.'
    });
...
app.get('/api/debug/push', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  try {
    const { verifyPushCredentials, getPushLog } = require('./services/push');
...
app.get('/api/debug/models', async (req, res) => {
  ...
  try {
    ...
    if (!apiKey) return res.json({ version: 'v294', keyPresent: false, reason: 'ANTHROPIC_API_KEY is NOT set on this server' });
  ...
  } catch (e) { res.json({ version: 'v294', error: e.message }); }
});

// GET /api/version - no auth; hit this in a browser to confirm which backend build is actually running
app.get('/api/version', (req, res) => {
  res.set('Cache-Control', 'no-store');
  res.json({ version: 'v294', built: '2026-06-13', note: 'v248 — manual AI parse runs in background (fixes 30s timeout).' });
});
...
async function startServer() {
  ...
  try {
    ...
    // QuickBooks IIF export tracking columns — in their OWN try block so they always run, independent of
    // whether any earlier migration in the assignment block failed. (These were defined on the WorkOrder
    // model but missing from the DB, which crashed the Invoiced-tab history query.)
    try {
